...
USUL_IMPLEMENT_IUNKNOWN_MEMBERS ( SmoothingFactory, SmoothingFactory::BaseClass );
...
{
  Usul::Interfaces::IClassFactory::ValidRefPtr factory ( new SmoothingFactory() );
...
SmoothingFactory::SmoothingFactory() : BaseClass()
{
}
...
Usul::Interfaces::IUnknown *SmoothingFactory::queryInterface ( unsigned long iid )
{
  switch ( iid )
  {
...
Usul::Interfaces::IUnknown *SmoothingFactory::createInstance ( unsigned long iid )
{
  // Require this to construct.
  SmoothingComponent::ValidAccessRefPtr component ( new SmoothingComponent() );
