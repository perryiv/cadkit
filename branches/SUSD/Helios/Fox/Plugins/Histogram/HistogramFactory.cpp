...
USUL_IMPLEMENT_IUNKNOWN_MEMBERS ( HistogramFactory, HistogramFactory::BaseClass );
...
{
  Usul::Interfaces::IClassFactory::ValidRefPtr factory ( new HistogramFactory() );
...
HistogramFactory::HistogramFactory() : BaseClass()
{
}
...
Usul::Interfaces::IUnknown *HistogramFactory::queryInterface ( unsigned long iid )
{
  switch ( iid )
  {
...
Usul::Interfaces::IUnknown *HistogramFactory::createInstance ( unsigned long iid )
{
  // Require this to construct.
  HistorgramComponent::ValidAccessRefPtr component ( new HistorgramComponent() );
