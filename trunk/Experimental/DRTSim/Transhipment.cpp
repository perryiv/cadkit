...
osg::Group*		Transhipment::buildTranshipmentScene( )
{
	...
	_root->addChild( _createTextAsNode( osg::Vec3( 200.0f, -30.0f, 0.0f), osg::Vec4( 1.0f, 0.5f, 0.5f, 1.0f ), 20.0f, " Transipment " ) ); 
...
osg::Sequence*	Transhipment::createTranshipmentSequence( )
{
	...
	if( !_tranDetails.empty() )
	{
		...
		for( unsigned int s = 0; s < _tranSteps.size(); ++s )
		{
			...
			{
				_tranSequence->addChild( _createTranshipmentDetailsbyStep( j, sId, eId ) );
