...
USUL_IMPLEMENT_IUNKNOWN_MEMBERS ( ErodeComponent, ErodeComponent::BaseClass );
...
ErodeComponent::ErodeComponent() : BaseClass()
{
}
...
Usul::Interfaces::IUnknown *ErodeComponent::queryInterface ( unsigned long iid )
{
  switch ( iid )
  {
...
std::string  ErodeComponent::menuText() const
{
  return "Erode Image";
}
...
std::string ErodeComponent::hotKeyText() const
{
  return "";
}
...
std::string ErodeComponent::statusBarText() const
{
  return "";
}
...
std::string ErodeComponent::menuGroup() const
{
...
void ErodeComponent::execute ( Usul::Interfaces::IUnknown *caller )
{
  ...
  if ( activeDocument.valid () )
  {
    ...
    {
      OsgTools::Images::erode( *image->getImage(), 3, 3 );
