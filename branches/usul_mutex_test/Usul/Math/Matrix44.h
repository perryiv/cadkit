...
template < class T, class I, class E > 
inline Usul::Math::Vector3<T,I,E> operator * 
  ( const Usul::Math::Vector3<T,I,E> &b, const Usul::Math::Matrix44<T,I,E> &M )
{
  typedef Usul::Math::Vector3<T,I,E> Vector3;

  const T d ( 1.0f / ( M ( 0, 3 ) * b[0]+M ( 1, 3 )* b[1] +M ( 2, 3 )* b[2] + M ( 3, 3 ) ) );
  return Vector3 ( ( M ( 0, 0 ) * b[0] + M ( 1, 0 ) * b[1] + M ( 2, 0 ) * b[2] + M ( 3, 0 ) ) * d,
                   ( M ( 0, 1 ) * b[0] + M ( 1, 1 ) * b[1] + M ( 2, 1 ) * b[2] + M ( 3, 1 ) ) * d,
                   ( M ( 0, 2 ) * b[0] + M ( 1, 2 ) * b[1] + M ( 2, 2 ) * b[2] + M ( 3, 2 ) ) * d);
}
