...
osg::Node *DRTSimDocument::buildScene ( const BaseClass::Options &options, Unknown *caller )
{
 ...
 // add code here (application) by Wei.CHEN
  ...
  // put "current step" and Agent & Stock color bars
  {
	...
	// area
	_root->addChild( _area.buildAreaScene( ) );

	// agent
	_root->addChild( _agent.buildAgentScene( ) );

	// stock
	_root->addChild( _stock.buildStockScene( ) );

	// transhipment
	_root->addChild( _trans.buildTranshipmentScene( ) );

	// spores
	_root->addChild( _spore.buildSporeScene( ) );
