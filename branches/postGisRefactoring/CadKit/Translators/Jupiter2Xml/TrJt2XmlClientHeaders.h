  const SlVec3f &           getValue() const { return _scale; }

  //////////////////////////////////////////////////////////////////////////
//
//  Clients include this header before others from this library.
//
//////////////////////////////////////////////////////////////////////////

#ifndef _CADKIT_JUPITER_TO_XML_LIBRARY_HEADERS_H_
#define _CADKIT_JUPITER_TO_XML_LIBRARY_HEADERS_H_

#ifdef _WIN32
# pragma warning(disable:4786) // Truncated debug names.
# pragma warning(disable:4251) // See note 3ce3a140-2edf-11d4-98a4-0040054c86c7.
#endif

// Needed headers.
#include "Standard/SlClientHeaders.h"

// Headers from this library.
#include "TrJt2Xml.h"

#ifdef _WIN32
# pragma warning(disable:4786) // Truncated debug names.
# pragma warning(disable:4251) // See note 3ce3a140-2edf-11d4-98a4-0040054c86c7.
#endif

#endif // _CADKIT_JUPITER_TO_XML_LIBRARY_HEADERS_H_
